...
[![License: MIT](https://img.shields.io/badge/license-MIT-green)](LICENSE) [![Emojery](https://api.emojery.app/badge/github/khasky/backend-architecture-playbook.svg)](https://emojery.app/react?t=github/khasky/backend-architecture-playbook)
...
- [Backend Architecture Playbook](#backend-architecture-playbook)
...
Your React app will branch on status codes and parse bodies. Agree on a **small, stable JSON shape** for error responses and describe it in OpenAPI (and in the [frontend playbook](https://github.com/khasky/frontend-architecture-playbook) the client should use the same assumptions).
...
- fail CI when the spec drifts from implementation (contract tests or codegen in the pipeline — see the [DevOps playbook](https://github.com/khasky/devops-delivery-playbook)).
...
**Default for new Node + TypeScript services:** **[Vitest](https://vitest.dev/)** for unit and HTTP integration tests (often with **Supertest** or the framework's inject helper). **Jest** is still a reasonable choice when you are extending an existing Jest-heavy repo; align the runner with the [frontend](https://github.com/khasky/frontend-architecture-playbook) and [DevOps](https://github.com/khasky/devops-delivery-playbook) playbooks so CI commands stay obvious (`vitest run` vs `npm test -- --runInBand`).
